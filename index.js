...
// --- 状态变量 ---
// 用于通知去重的时间戳，记录上次发送通知（包括测试通知）的时间
    ...
    async loadInterface() {
        try {
            ...
            if (!$(`link[href="${cssPath}"]`).length) {
                 // 使用 fetch API 尝试获取 CSS 文件。fetch 不会跨域加载本地文件，但在同源下工作良好
                 fetch(cssPath)
                     .then(response => {
...
const EventHandler = {
     ...
     * - !! 关键：发送测试通知/声音时会检查并遵守通知间隔设置 !!
     * - !! 关键：成功发送测试通知/声音后会更新 lastNotificationTimestamp !!
     * - 同步更新通知总开关 (#enableNotification) 的状态并保存
     */
